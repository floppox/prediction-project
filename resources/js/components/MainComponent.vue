<template>
  <div>
    <md-toolbar class="md-primary">
      <h1 class="md-title">Prediction Project</h1>
    </md-toolbar>

    <md-content>
      <md-table md-card>
        <md-table-row>
          <md-table-head md-numeric>Position</md-table-head>
          <md-table-head>Club</md-table-head>
          <md-table-head>Played</md-table-head>
          <md-table-head>Won</md-table-head>
          <md-table-head>Drawn</md-table-head>
          <md-table-head>Lost</md-table-head>
          <md-table-head>GF</md-table-head>
          <md-table-head>GA</md-table-head>
          <md-table-head>GD</md-table-head>
          <md-table-head>Points</md-table-head>
        </md-table-row>

        <md-table-row
            v-for="tournamentTableRow in  tournamentTable"
            :data="tournamentTableRow"
            :key="tournamentTableRow.position"
        >
          <md-table-cell md-numeric>{{ tournamentTableRow.position || '&mdash;'}}</md-table-cell>
          <md-table-cell>{{ tournamentTableRow.club.name  }}</md-table-cell>
          <md-table-cell>{{ tournamentTableRow.played || 0 }}</md-table-cell>
          <md-table-cell>{{ tournamentTableRow.won || 0  }}</md-table-cell>
          <md-table-cell>{{ tournamentTableRow.drawn || 0  }}</md-table-cell>
          <md-table-cell>{{ tournamentTableRow.lost || 0  }}</md-table-cell>
          <md-table-cell>{{ tournamentTableRow.gf || 0  }}</md-table-cell>
          <md-table-cell>{{ tournamentTableRow.ga || 0  }}</md-table-cell>
          <md-table-cell>{{ tournamentTableRow.gd || 0  }}</md-table-cell>
          <md-table-cell>{{ tournamentTableRow.points || 0  }}</md-table-cell>
        </md-table-row>
      </md-table>

      <md-button
          v-if="tournamentFinished"
          class="md-raised md-primary centered"
          @click="startNewSeason()"
      >
        Start new season
      </md-button>

      <md-button
          v-else
          class="md-raised md-primary centered"
          @click="playNextTour()"
      >
        Play next tour
      </md-button>

      <md-table md-card v-if="tour[0]">
        <md-table-head>
          <h3 class="md-title">Last tour results</h3>
        </md-table-head>
        <md-table-row
            v-for="meet in tour"
            :data="meet"
            :key="meet.id"
        >
          <md-table-cell>{{ meet.clubs[0].name}}</md-table-cell>
          <md-table-cell>{{ meet.clubs[0].pivot.score}}</md-table-cell>
          <md-table-cell>{{ meet.clubs[1].pivot.score}}</md-table-cell>
          <md-table-cell>{{ meet.clubs[1].name}}</md-table-cell>
        </md-table-row>
      </md-table>

      <md-table md-card>
        <md-table-head>
          <h3 class="md-title">Championship Probability Prediction</h3>
        </md-table-head>
        <md-table-row
            v-for="predictionRow in prediction"
            :data="predictionRow"
            :key="predictionRow.club_name"
        >
          <md-table-cell>{{ predictionRow.club_name }}</md-table-cell>
          <md-table-cell>{{ predictionRow.championship_probability}}%</md-table-cell>

        </md-table-row>
      </md-table>

    </md-content>

    <div class="loading-overlay" v-if="loading">
      <md-progress-spinner md-mode="indeterminate" :md-stroke="2"></md-progress-spinner>
    </div>

  </div>
</template>

<script>
export default {
  name: "App",
  data() {
    return {
      loading: false,
      tournamentTable: [],
      lastTourNumber: -1,
      tour: [],
      prediction: [],
      tournamentFinished: false
    };
  },

  created() {
    this.loading = true;

    axios.get('/api/tournament/table')
        .then(({data}) => this.tournamentTable = data.data)
        .then(() => this.loading = false)
  },

  watch: {
    tournamentTable() {
      this.lastTourNumber = +this.tournamentTable[0].played
    },

    tour() {
      this.lastTourNumber = this.tour[0] ? this.tour[0].tour_number : 0;
      if ((this.tournamentTable.length - 1 ) * 2 === this.lastTourNumber) {
        this.tournamentFinished = true
      }
    },

    lastTourNumber() {
      axios.get('/api/tournament/table')
          .then(({data}) => this.tournamentTable = data.data)

      axios.get('/api/tour/' + this.lastTourNumber)
          .then(({data}) => this.tour = data.data)

      axios.get('/api/prediction')
          .then(({data}) => this.prediction = data.data)
    },

    tournamentFinished() {
      if(this.tournamentFinished) {
        alert('The tournament is finished')
      }
    }
  },

  computed: {},

  methods: {
    startNewSeason() {
      this.loading = true;

      axios.post('/api/tournament/toss')
          .then(({data}) => this.tournamentTable = data.data)
          .then(() => this.tournamentFinished = false)
          .then(() => this.loading = false);
    },

    playNextTour() {
      this.loading = true;

      axios.post('/api/tour/play')
          .then((response) => response.data)
          .then((data) => this.tour = data.data)
          .catch((error) => this.tournamentFinished = true)
          .then(() => this.loading = false);

    },
  },

};
</script>

<style lang="scss">
body {
  background: url(data:image/svg+xml;base64,);
}

.md-content {
  z-index: 1;
  padding: 40px;
  width: 100%;
  max-width: 1200px;
  margin: 20px auto;
  position: relative;

}

.md-content.md-theme-default {
  background: transparent;
  border: 0;
}

.loading-overlay {
  z-index: 10;
  top: 0;
  left: 0;
  right: 0;
  position: absolute;
  width: 100%;
  height: 100%;
  background: rgba(255, 255, 255, 0.9);
  display: flex;
  align-items: center;
  justify-content: center;
}

.md-card {
  min-width: 320px;
  margin: 6px;
  vertical-align: top;
}

.centered {
  display: block;
  margin: 40px auto 20px;
}
</style>
